...
build_effis_flourish_export <- function(data) {
  data.frame(
    id = data$id,
    gegevenstype = rep("EFFIS-brandgebied", nrow(data)),
    ...
    firms_hotspot = rep("Nee", nrow(data)),
    satellieten = ifelse(
...
build_effis_actuality_summary <- function(data) {
  categories <- data.frame(
    ...
    actualiteitvolgorde = c(1L, 2L, 3L, 4L),
...
build_effis_metadata <- function(data, download) {
  ...
  list(
    ...
    belangrijke_beperking = paste(
      "EFFIS-datums zijn kaartregistraties en niet noodzakelijk de echte",
      "ontstaans- of einddatum van een brand."
    )
  )
}
...
run_effis_pipeline <- function(
  ...
  message(
    ...
    " EFFIS-brandgebieden weggeschreven naar ",
